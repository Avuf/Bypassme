...
async def loopthread(client, message):
    ...
    if bypasser.ispresent(ddllist, urls[0]):
        msg = await app.send_message(message.chat.id, "⚡ __generating... Please Wait 8-10 Seconds__", reply_to_message_id=message.message_id)
    else:
        if urls[0] in "https://olamovies" or urls[0] in "https://psa.pm/":
            msg = await app.send_message(message.chat.id, "🔎 __this might take some time...__", reply_to_message_id=message.message_id)
        else:
            msg = await app.send_message(message.chat.id, "🔎 __bypassing... Please Wait 8-10 Seconds__", reply_to_message_id=message.message_id)
    ...
    try:
        await app.edit_message_text(message.chat.id, msg.message_id, f'**__Ads Link - {urls}__**\n\n**__Original Link - {link}__**\n**__Generated By - [Link Bypasser](https://t.me/TellyLinkBypasser_bot)\n\n⚡Powered By - @TellyBotzz__**', disable_web_page_preview=True)
        await app.send_message(log_channel, text=f"**__Ads Link - {urls}__**\n\n**__Original Link - {link}__**\n**__Generated By - [Link Bypasser](https://t.me/TellyLinkBypasser_bot)__**\n\n**__Requested By: {message.from_user.mention}__**\n**__First Name: {message.from_user.first_name}__**\n**__Username: @{message.from_user.username}__**\n**__ID: {message.from_user.id}__**\n\n**__#BypassLinks__**")
    except:
        try:
            await app.edit_message_text(message.chat.id, msg.message_id, "__Failed to Bypass__",
                                        reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚡Complaint⚡", url="https://t.me/Legend_Shivam_7Bot")]]))
        except:
            try:
                await app.delete_messages(message.chat.id, msg.message_id)
            except:
                pass
            await app.send_message(message.chat.id, "__Failed to Bypass__",
                                   reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("⚡Complaint⚡", url="https://t.me/Legend_Shivam_7Bot")]]))
