...
async def loopthread(client, message):
    ...
    if bypasser.ispresent(ddllist, urls[0]):
        msg = await app.send_message(message.chat.id, "⚡ __generating... Please Wait 8-10 Seconds__", reply_to_message_id=message.id)
    else:
        if urls[0] in "https://olamovies" or urls[0] in "https://psa.pm/":
            msg = await app.send_message(message.chat.id, "🔎 __this might take some time...__", reply_to_message_id=message.id)
        else:
            msg = await app.send_message(message.chat.id, "🔎 __bypassing... Please Wait 8-10 Seconds__", reply_to_message_id=message.id)
    ...
    try:
        ...
        try:
            await app.edit_message_text(message.chat.id, msg.id, "__Failed to Bypass__",
